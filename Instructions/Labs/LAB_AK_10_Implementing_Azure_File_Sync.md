...
1. **스토리지 계정 만들기** 페이지의 **기본 사항** 탭에서 다음 설정을 지정합니다.
   ...
   >**참고:** 전역적으로 고유할 수 있는 이름을 선택하세요. 예를 들어 \<*yourlowercaseinitials*>*DDMMYY* 형식으로 스토리지 계정 이름을 지정할 수 있습니다. 예를 들어 이름이 Devon Torres이고 2022년 1월 30일에 스토리지 계정을 만드는 경우 스토리지 계정 이름은 **dt013022** 가 됩니다. 해당 이름을 이미 사용 중인 경우 이름을 사용할 수 있게 될 때까지 이름에 다른 문자를 추가합니다. 

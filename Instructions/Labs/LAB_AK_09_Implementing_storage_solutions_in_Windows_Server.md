...
1. **SConfig** 메뉴가 표시되는 경우, **Enter 번호에서 옵션을 선택하려면** **15** 를 입력하고 Enter 키를 눌러 **PowerShell** 콘솔 세션으로 종료합니다.
...
1. **sea-svr1.contoso.com** 페이지의 **도구** 메뉴에서 **PowerShell** 을 선택한 다음 메시지가 표시되면 암호로 **Pa55w.rd** 를 사용하여 **CONTOSO\\Administrator** 사용자로 로그인합니다.
...
1. **SConfig** 메뉴가 표시되는 경우, **옵션을 선택할 Enter 번호에서** **15** 를 입력하고 Enter 키를 눌러 **PowerShell** 콘솔 세션으로 종료합니다.
...
1. **프로비저닝 입력 지정** 페이지에서 **씬** 을 선택한 후 **다음** 을 선택합니다.
...
1. **장애 조치(failover) 클러스터 관리자** 창에서 **S2D-SOFS** 를 선택한 다음 **공유** 탭을 선택합니다.
...
1. **장애 조치(failover) 클러스터 관리자** 로 전환하고 **디스크** 를 선택한 다음 **CSV(클러스터 가상 디스크)** 를 선택합니다.
